...
def get_full_graph(limit: int = 500):
    """Get the full knowledge graph from Neo4j (limited nodes) for visualization."""
    service = GraphService()
    try:
        return service.get_full_graph(limit=limit)
    finally:
        service.close()


@router.get("/subgraph/{entity_name}", response_model=Dict[str, Any])
def get_subgraph(entity_name: str, depth: int = 2):
    """Get the neighborhood graph around a specific entity from Neo4j."""
    service = GraphService()
    try:
        return service.query_subgraph(entity_name, depth=depth)
    finally:
        service.close()
